 ...
 *
 * <p>This class allows users to perform the following actions:
 * <ul>
...
public class DiaryEditUI {
  ...
  /**
   * Prompts the user to enter a title, which adds a {@link RecipeLabel}
   * to an existing {@link DiaryEntry}.
   *
   * <p>If successful, calls to {@link DiaryService#addLabel(DiaryEntry, RecipeLabel)}.
   *
  ...
  public void addLabelToDiary(Scanner input) {
    ...
    List<DiaryEntry> diaryEntries =
        RegisterHandler.getDiaryRegister().getDiaryEntriesByAuthor(UserService.getCurrentUser());
    if (diaryEntries.isEmpty()) {
      System.out.println(
          "You have not written any recipes yet, "
              + "please write a recipe first before adding a label."
      );
      return;
    }
    ...
    // Get the diary entry.
    DiaryEntry diaryEntry = RegisterHandler.getDiaryRegister()
        .getDiaryEntryByTitleAndAuthor(title, UserService.getCurrentUser());
    ...
    if (labelAdded) {
      System.out.println(
          "Successfully added label " + label.formattedLabel()
              + " to " + diaryEntry.getTitle() + "."
      );
    } else {
  ...
  /**
   * Prompts the user to enter a title, which removes
   * an existing {@link RecipeLabel} from a {@link DiaryEntry}.
   *
   * <p>If successful, calls to {@link DiaryService#removeLabel(DiaryEntry, RecipeLabel)}.
   *
   * @param input a {@link Scanner} object to read user input.
   */
  public void removeLabelFromDiary(Scanner input) {
    ...
    if (diaryEntries.isEmpty()) {
      System.out.println(
          "You have not written any recipes yet,"
              + " please write a recipe first before removing a label."
      );
      return;
    }
    ...
    if (labelExists) {
      System.out.println(
          "Successfully removed label "
              + label.formattedLabel() + " from " + diaryEntry.getTitle() + "."
      );
    } else {
  ...
  /**
   * Prompts the user to enter a title and a specific line number,
   * which changes the content of a {@link DiaryEntry}.
   *
   * <p>If successful, calls to {@link DiaryService#editContentLine(DiaryEntry, int, String)}.
   *
   * @param input a {@link Scanner} object to read user input.
   */
  public void editContentLine(Scanner input) {
    ...
    System.out.println(
        "Successfully changed line "
            + lineNumber + " of " + diaryEntry.getTitle() + " to: " + newContent
    );
  }
