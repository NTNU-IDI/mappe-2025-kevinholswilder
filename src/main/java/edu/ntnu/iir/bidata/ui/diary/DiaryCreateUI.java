...
/**
 * This class provides the method to create {@link DiaryEntry} objects
 * and adding them to the {@link DiaryRegister}.
 *
...
public class DiaryCreateUI {
   ...
   *
   * <p>If successful, calls to {@link DiaryService#createDiaryEntry(String, String, Author, List)}.
   *
  ...
  public void writeDiary(Scanner input) {
    ...
    while (true) {
      ...
      boolean titleExists = RegisterHandler.getDiaryRegister()
          .getDiaryEntryByTitleAndAuthor(title, UserService.getCurrentUser()) == null;
      if (!titleExists) {
    ...
    System.out.println(
        "Write the content for your cooking diary:\nType 'exit' to save and finish writing.");
    ...
    DiaryEntry diaryEntry =
        DiaryService.getInstance().createDiaryEntry(title, content, author, labels);
    System.out.println(
        "Successfully created cooking diary with title: " + diaryEntry.getTitle() + ".");
  }
