...
/**
 * This class provides the method for deleting {@link DiaryEntry}
 * objects from the {@link DiaryRegister}.
 *
...
public class DiaryDeleteUI {
  ...
  /**
   * Prompts the user to enter the title of the {@link DiaryEntry}
   * they wish to delete from themselves.
   *
   * <p>If a {@link DiaryEntry} is found for the {@link Author},
   * it proceeds to call {@link DiaryService#deleteDiaryEntry(String, Author)}.
   *
  ...
  public void deleteDiary(Scanner input) {
    ...
    if (diaryEntries.isEmpty()) {
      System.out.println(
          "You have not written any recipes yet, "
              + "please write a recipe first before attempting to delete one."
      );
      return;
    }
    ...
    boolean diaryExists =
        DiaryService.getInstance().deleteDiaryEntry(title, UserService.getCurrentUser());
    if (diaryExists) {
